    ...
    for n,individual_1 in enumerate(population):
        for n,individual_2 in enumerate(population):
            if individual_1 is not individual_2:
                assert δ(individual_1,individual_2) > 10       

def test_evolution_2():
    evoMan = EvoManager()
...
def test_speciation_3():
    ...
    # Mutate all links randomly 
    species = evoMan.speciate(population)
    assert len(species) == 1
    print(len(species),len(species[0]))
    assert len(species[0]) == population_size
